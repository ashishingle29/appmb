...
//   const res = await fetch(`${siteMetadata?.siteUrl}/api/tags`);
...
const TagPage = async ({ params }) => {
  ...
  try {
    if (!siteMetadata?.siteUrl) throw new Error("siteUrl is undefined in siteMetadata");

    // Fetch all categories
    const getCategory = await fetch(`${siteMetadata.siteUrl}/api/tags`);

    if (!getCategory.ok) {
      throw new Error(`Failed to fetch categories: ${getCategory.status} ${getCategory.statusText}`);
    }

    const allCat = await getCategory.json();

    allCategories = [...allCategories, ...allCat.map((cat) => cat)];
  ...
  try {
    // Fetch blogs based on tag slug
    const getBlogs = await fetch(`${siteMetadata.siteUrl}/api/posts?tag=${slug}`);

    if (!getBlogs.ok) {
      throw new Error(`Failed to fetch blogs: ${getBlogs.status} ${getBlogs.statusText}`);
    }
...
export async function generateStaticParams() {
  try {
    ...
    return [{ slug: "all" }]; // Return default path in case of failure
  }
}
